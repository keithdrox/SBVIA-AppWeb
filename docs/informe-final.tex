...
\begin{titlepage}
    ...
    Jefferson M. Umaginga Arévalo \quad (ORCID: \href{https://orcid.org/0009-0005-9831-0801}{0009-0005-9831-0801}) \\
...
\textbf{Main results:} Five hot-cache k6 runs recorded $p95$ values between 29.35 and 126.41 ms with no errors; a separate 10-cycle experiment observed an approximate 1.2x speedup. JaCoCo reported $79.73\%$ line coverage and $70.41\%$ branch coverage. The mean SUS score was $84.17/100$ ($SD=8.64$; $N=15$). \\
...
La elicitación combinó revisión del alcance académico, entrevistas semiestructuradas y análisis del sistema existente. Cada necesidad fue transformada en una declaración verificable, procurando que contuviera sujeto, obligación, acción, objeto y condición. La redacción normativa usa ``deberá'' para separar requisitos de explicaciones de diseño, de acuerdo con ISO/IEC/IEEE 29148 y las prácticas de especificación de Wiegers y Beatty \cite{iso29148,wiegers2013software}. Las historias siguen el enfoque de Cohn y los casos de uso describen interacciones observables conforme a Cockburn \cite{cohn2004user,cockburn2001writing}. La prioridad Must indica que el requisito es indispensable para la versión final; no implica que esté automáticamente verificado.
...
La población objetivo de la evaluación de usabilidad corresponde a conductores en formación capaces de utilizar un navegador web moderno. La muestra disponible comprende 15 participantes, identificados exclusivamente mediante los códigos P01--P15. La selección fue no probabilística por conveniencia: se invitó a personas accesibles para el equipo dentro del contexto académico. Este procedimiento permite una evaluación formativa del prototipo, pero introduce sesgo de selección y no garantiza representatividad por edad, experiencia vial, ubicación geográfica o familiaridad tecnológica \cite{baltes2022sampling}.

En consecuencia, los resultados SUS describen únicamente a la muestra observada. No se extrapolan a la población nacional de conductores ni se interpretan como evidencia de reducción de siniestros. Los archivos públicos no incluyen edad, género, dispositivo ni otros atributos demográficos; por ello, el informe tampoco afirma diversidad sobre variables que no fueron registradas.
...
Para usabilidad, $H_{0U}$ establece que la media poblacional SUS no supera el umbral de 68 y $H_{1U}$ que lo supera. Debido al tamaño y tipo de muestreo, la decisión se presenta mediante media, mediana, desviación típica e intervalo de confianza con $t$ de Student; la puntuación no se denomina porcentaje. Para cobertura, seguridad y Lighthouse se aplican criterios de aceptación previamente fijados y no pruebas de hipótesis retrospectivas.
...
Para SUS, cada participante completa una sesión guiada de aproximadamente 20 minutos y responde inmediatamente los diez ítems estándar en escala Likert de 1 a 5. Los ítems impares aportan $respuesta-1$ y los pares $5-respuesta$; la suma se multiplica por 2.5. El archivo crudo conserva respuestas y código anónimo, mientras el cuaderno recalcula puntuaciones, dispersión e intervalo. Cualquier aprobación institucional y consentimiento firmado se custodia fuera del repositorio público; su existencia debe comprobarse documentalmente antes de presentar el estudio como aprobado.
...
La Figura~\ref{fig:flujo-mvc} resume el recorrido de una solicitud a través de las capas implementadas. La separación mostrada permite ubicar la validación HTTP en el controlador, las reglas en el servicio y el acceso durable en los repositorios, evitando que la interfaz dependa directamente del motor de datos.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.92\textwidth]{diagramas/flujo-mvc-springboot.png}
    \caption{Flujo MVC implementado entre Angular, Spring Boot, Redis y PostgreSQL. Fuente: elaboración propia a partir de la arquitectura versionada de SBVIA.}
    \label{fig:flujo-mvc}
\end{figure}
...
La Figura~\ref{fig:dashboard} presenta el dashboard que concentra las rutas principales del conductor. Las mejoras recientes añadieron el landmark \texttt{main}, ajustaron el contraste del login y eliminaron dependencias tipográficas externas. La última corrida local obtuvo 100 en accesibilidad, buenas prácticas y SEO, además de 86 en rendimiento; el protocolo definitivo aún debe repetirse sobre la URL pública.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.94\textwidth]{capturas/dashboard.png}
    \caption{Dashboard principal de SBVIA con accesos a escenarios, prácticas y perfil. Fuente: captura propia del sistema en ejecución.}
    \label{fig:dashboard}
\end{figure}
...
\begin{table}[htbp]
\centering
\caption{Resultados de latencia k6 (5 corridas de caché caliente)}
...
La Figura~\ref{fig:k6-boxplot} conserva la distribución observada en las corridas crudas y permite detectar dispersión que una única media ocultaría. En particular, la cuarta corrida presenta mayor variabilidad, aunque permanece dentro del criterio de aceptación.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.88\textwidth]{mediciones/perf/k6-boxplot.png}
    \caption{Distribución de latencias de las corridas k6 versionadas. Fuente: elaboración propia mediante \texttt{scripts/gen-k6-boxplot.py}.}
    \label{fig:k6-boxplot}
\end{figure}

\section{Usabilidad del Sistema (SUS)}
En la prueba con $N = 15$ participantes se obtuvo una media SUS de $84.17$ ($DT = 8.64$, $IC\ 95\% = [79.38, 88.95]$), lo cual posiciona al sistema en la categoría adjetival ``Bien+'' (Grado B+) según los intervalos de interpretación adoptados a partir de Bangor, Kortum \& Miller (2008).
...
La Figura~\ref{fig:sus-distribution} muestra tanto la distribución como los valores individuales recalculados. La línea de media coincide con 84.17 y permite comprobar visualmente que todos los participantes superaron el umbral de aceptación de 68 puntos.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.96\textwidth]{mediciones/sus/sus-distribution.png}
    \caption{Distribución y puntuaciones individuales de la escala SUS para 15 participantes. Fuente: elaboración propia mediante \texttt{scripts/sus-analysis.ipynb}.}
...
\begin{itemize}
    ...
    \item \textbf{Lighthouse:} Rendimiento: $86\%$, Accesibilidad: $100\%$, Buenas Prácticas: $100\%$, SEO: $100\%$, según la última medición completa conservada en \texttt{report-realtime.json}.
\end{itemize}
...
Los resultados se clasifican en tres grupos. Se consideran cumplidos el protocolo de cinco corridas calientes, el umbral de latencia, la tasa de error observada, el objetivo SUS, las coberturas de líneas y ramas y los controles funcionales documentados. Se consideran parciales la comparación estadística frío/caliente, la descarga completa de certificado y el protocolo Lighthouse sobre una URL pública con seis corridas. Permanecen externos al código la firma de aprobación, el despliegue HTTPS verificable, el video, la defensa y los DOI.
...
\begin{itemize}
    ...
    \item \textbf{Respuesta a RQ3:} La media SUS de $84.17$ (Grado B+, ``Bien+'') confirma una usabilidad aceptable para conductores en formación, con margen de mejora en el diseño de interfaz.
\end{itemize}
...
SBVIA integra una base funcional y reproducible con Spring Boot, Angular, PostgreSQL, Redis y Docker, acompañada de requisitos trazables, pruebas, mediciones y controles de seguridad. La evidencia disponible confirma cinco corridas de caché caliente dentro del umbral, una cobertura de $79.73\%$ de líneas y $70.41\%$ de ramas, y una corrida Lighthouse local con puntuaciones completas salvo rendimiento ($86\%$). La entrega todavía requiere producir cinco corridas comparables de caché fría, ejecutar las seis auditorías Lighthouse sobre una URL pública y verificar el despliegue HTTPS, los DOI y la aprobación documental. En consecuencia, el artefacto se considera técnicamente avanzado, pero no se declara culminado hasta cerrar y volver a verificar esos criterios.
...
\textbf{Disponibilidad de Código:} El código fuente está disponible públicamente en \url{https://github.com/keithdrox/SBVIA-AppWeb}. La etiqueta definitiva \texttt{v1.0.0} deberá actualizarse al commit aprobado únicamente después del cierre de todos los criterios de entrega. Los metadatos de publicación se preparan siguiendo los principios de citación de software \cite{smith2016software}.
...
\begin{table}[htbp]
\centering
\caption{Criterios de aceptación de instalación y actualización}
...
\begin{table}[htbp]
\centering
\caption{Secuencia mínima de aceptación funcional}
...
\begin{table}[htbp]
\centering
\caption{Amenazas principales y controles comprobables}
...
\begin{table}[htbp]
\centering
\caption{Paquete mínimo de evidencia para la entrega}
...
Usabilidad & Respuestas SUS anónimas & Media, mediana, DT e IC 95\% \\
