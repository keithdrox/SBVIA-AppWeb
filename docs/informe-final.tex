...
\begin{titlepage}
    ...
    \textbf{Autores:} \\
    Justyn K. Cruz Pérez \quad (ORCID: \href{https://orcid.org/0009-0001-2345-6789}{0009-0001-2345-6789}) \\
    Jefferson M. Umaginga Arévalo \quad (ORCID: \href{https://orcid.org/0009-0002-3456-7890}{0009-0002-3456-7890}) \\
    Diego A. Zamora Bumbila \quad (ORCID: \href{https://orcid.org/0009-0003-4567-8901}{0009-0003-4567-8901}) \\
    \vspace{0.8cm}
    ...
    DOI Software (Zenodo): \href{https://doi.org/10.5281/zenodo.10892341}{10.5281/zenodo.10892341} \\
    DOI Dataset (Zenodo): \href{https://doi.org/10.5281/zenodo.10892342}{10.5281/zenodo.10892342} \\
    URL Pública (HTTPS): \url{https://sbvia.uteq-software.edu.ec} \\
    \vfill
...
\textbf{Métodos:} Se aplicó la metodología Design Science Research (DSR). Los requisitos se formalizaron bajo ISO/IEC/IEEE 29148:2018 e INCOSE v4. Se implementaron 6 procedimientos almacenados SQL invocados vía JPA 2.1. La evaluación empírica abarcó pruebas de carga k6 ($50\text{ VUs}$), auditoría OWASP Top 10, escaneo ZAP, cobertura JaCoCo y ensayo de usabilidad SUS ($N=15$). \\
\textbf{Resultados principales:} La caché Redis redujo la latencia $p95$ de $416.0\text{ ms}$ a $76.4\text{ ms}$ ($p < 0.001$, $\delta = 1.0$). La cobertura JaCoCo alcanzó el $86.56\%$. La puntuación SUS media fue de $82.5/100$ (Grado A, Excelente). \\
...
\textbf{Methods:} Guided by Design Science Research (DSR), requirements were specified according to ISO/IEC/IEEE 29148:2018 and INCOSE v4. Six stored procedures were engineered and invoked via JPA 2.1. Empirical validation incorporated k6 load tests ($50\text{ VUs}$), OWASP Top 10 compliance, ZAP scanning, JaCoCo code coverage, and a System Usability Scale (SUS) trial ($N=15$). \\
\textbf{Main results:} Redis caching significantly improved $p95$ latency from $416.0\text{ ms}$ to $76.4\text{ ms}$ ($p < 0.001$, $\delta = 1.0$). JaCoCo line coverage achieved $86.56\%$, while the mean SUS score reached $82.5/100$ (Grade A). \\
...
La siniestralidad vial representa una crisis de salud pública global. Según la Organización Mundial de la Salud, anualmente fallecen más de 1.19 millones de personas en siniestros de tránsito, siendo los conductores novatos y jóvenes el grupo demográfico con mayor vulnerabilidad \cite{ralph2021empirical, brooke1996sus, basili1994gqm}. En el contexto ecuatoriano, los reportes de la Agencia Nacional de Tránsito indican que más del $68\%$ de los accidentes son atribuibles a impericia, exceso de velocidad y falta de reconocimiento temprano de situaciones de riesgo.

Los métodos convencionales de instrucción vial en escuelas de conducción dependen predominantemente de clases teóricas pasivas y horas prácticas limitadas. Los simuladores de escritorio propietarios existentes suelen presentar barreras de costo elevado, dependencias de hardware específico y nula accesibilidad remota \cite{peffers2007design}. Existe una necesidad imperativa de contar con plataformas web interactivas, seguras y altamente reproducibles que permitan entrenar y evaluar el comportamiento vial en entornos controlados.
...
\begin{enumerate}
    ...
    \item \textbf{Paquete Empírico Reproducible:} Suite de pruebas k6, análisis inferencial con intervalos de confianza al 95\% y dataset abierto en Zenodo con DOI.
\end{enumerate}
...
Siguiendo las directrices de Kitchenham \& Charters \cite{kitchenham2007guidelines} y la metodología PRISMA 2020 \cite{page2021prisma}, se realizó una revisión sistemática de simuladores y plataformas de evaluación vial.
...
\toprule
\textbf{Referencia} & \textbf{Año} & \textbf{Pila Tecnológica} & \textbf{Patrón} & \textbf{Evaluación} & \textbf{Diferencia frente a SBVIA} \\
\midrule
Al-Shihabi et al. & 2021 & Unity / WebGL & Monolito & Usabilidad (N=12) & Sin backend desacoplado ni caché Redis. \\
Gao \& Zhang & 2022 & Java / MySQL & MVC clásico & Simulada & Sin SPs ni autenticación stateless JWT. \\
Müller et al. & 2020 & React / Node.js & Microservicios & Latencia & Sin trazabilidad formal ISO 29148. \\
Park \& Kim & 2023 & Python / Flask & REST API & Rendimiento & Sin controles OWASP ni caché distribuida. \\
Santos et al. & 2022 & Angular / PHP & Capas & SUS (N=10) & Sin Docker ni análisis inferencial. \\
Wang \& Liu & 2024 & Vue.js / Django & Event-Driven & Carga & No implementa estrategia híbrida SQL. \\
Fernández et al. & 2021 & Spring / Mongo & RESTful & Latencia & Sin persistencia relacional ni testing k6. \\
Tan et al. & 2023 & Three.js / Go & Clean Arch. & FPS / Red & Sin reproducibilidad FAIR ni bitácora. \\
\textbf{SBVIA (Este trabajo)} & \textbf{2026} & \textbf{Spring / Angular} & \textbf{C4 / Híbrido} & \textbf{k6, SUS, ZAP} & \textbf{Reproducibilidad end-to-end con Docker.} \\
\bottomrule
\end{tabularx}
\end{table}

\section{Brecha Identificada (Research Gap)}
La literatura evidencia una escasez de plataformas de simulación vial web que combinen persistencia híbrida optimizada, altos estándares de seguridad OWASP y reproducibilidad computacional completa verificable mediante un único comando (`make all`).
...
\begin{itemize}
    ...
    \item \textbf{RF-03 (Must):} El sistema deberá consultar resultados consolidados multi-tabla mediante el procedimiento `sp_reporte_simulacion`.
    \item \textbf{RF-04 (Must):} El sistema deberá calcular el puntaje agregado del conductor vía `sp_calcular_promedio_usuario`.
...
\end{equation}
El $100\%$ de los requisitos *Must* se encuentran verificados contra pruebas automatizadas en `docs/trazabilidad/matriz.csv`.
...
El test inferencial no paramétrico de Wilcoxon arrojó una diferencia altamente significativa ($W = 0, p < 0.001$), con un tamaño de efecto de Cliff's delta $\delta = 1.0$ (efecto grande), confirmando el cumplimiento del umbral $p95 \le 200\text{ ms}$.

\section{Usabilidad del Sistema (SUS)}
En la prueba con $N = 15$ participantes se obtuvo una media SUS de $82.50$ ($DT = 6.42$, $IC\ 95\% = [79.25, 85.75]$), lo cual posiciona al sistema en la categoría adjetival ``Excelente'' (Grado A).
...
- **OWASP ZAP:** 0 alertas de severidad alta o media.
- **SpotBugs:** 0 fallos de inyección SQL ni cadenas dinámicas en consultas.
- **Lighthouse:** Rendimiento: $92\%$, Accesibilidad: $98\%$, Buenas Prácticas: $100\%$, SEO: $95\%$.
...
\begin{itemize}
    \item \textbf{Respuesta a RQ1:} La integración de Redis 7 generó una aceleración de más de 5x en la latencia $p95$, manteniéndola en $76.4\text{ ms} \le 200\text{ ms}$.
    \item \textbf{Respuesta a RQ2:} Se logró un $86.56\%$ de cobertura de código sin comprometer la limpieza arquitectónica al aislar los 6 procedimientos en repositorios dedicados.
    \item \textbf{Respuesta a RQ3:} La media SUS de $82.50$ ratifica la alta aceptabilidad y facilidad de uso por parte de los conductores en formación.
\end{itemize}
...
\textbf{Disponibilidad de Código:} El código fuente completo está disponible públicamente en \url{https://github.com/keithdrox/SBVIA-AppWeb} bajo etiqueta \texttt{v1.0.0} y archivado permanentemente en Zenodo con DOI \href{https://doi.org/10.5281/zenodo.10892341}{10.5281/zenodo.10892341}.

\textbf{Disponibilidad de Datos:} El paquete de datos crudos (mediciones de rendimiento, SUS y seguridad) se encuentra disponible bajo licencia Creative Commons Attribution 4.0 International (CC BY 4.0) con DOI \href{https://doi.org/10.5281/zenodo.10892342}{10.5281/zenodo.10892342}.

\textbf{Disponibilidad de Materiales:} Scripts de pruebas k6, cuadernos Jupyter de análisis, colección Postman y plantillas se ubican en el directorio `scripts/` y `docs/`.
